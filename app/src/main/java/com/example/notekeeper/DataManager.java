...
public class DataManager {
    ...
    private static void loadNotesFromDb(Cursor notesQuery) {
        ...
        int _IdPos = notesQuery.getColumnIndex(NoteInfoEntry._ID);
        ...
        while (notesQuery.moveToNext()){
            ...
            String _Id = notesQuery.getString(_IdPos);

            CourseInfo course = dataManager.getCourse(id);
            NoteInfo note = new NoteInfo(_Id, course, title, text);
    ...
    private static void loadCoursesFromDb(Cursor courseQuery) {
        DataManager dataManager = DataManager.getInstance();
        int colIdPos = courseQuery.getColumnIndex(CourseInfoEntry.COL_COURSE_ID);
        int colTitlePos = courseQuery.getColumnIndex(CourseInfoEntry.COL_COURSE_TITLE);
        dataManager.mCourses.clear();

        while (courseQuery.moveToNext()){
            String id = courseQuery.getString(colIdPos);
            String title = courseQuery.getString(colTitlePos);

            CourseInfo course = new CourseInfo(id, title, null);
